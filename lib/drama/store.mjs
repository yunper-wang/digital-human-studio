        ...
        for (const shot of project.shots || []) {
          ...
          if (shot.clip?.status === "generating") {
            ...
            shot.clip = {
              ...
              // 保留 audio 引用：配音轨独立于视频生成，交由下方 audio 孤儿检查处理
              audio: shot.clip.audio ?? null,
              error: { code: "CLIP_INTERRUPTED", message: "服务重启导致视频生成中断，可重新生成" }
            };
          }
          if (shot.clip?.audio?.status === "generating") {
            // 与 clip 同理：配音生成是进程内的，磁盘上的 generating 必然是孤儿
            shot.clip.audio = {
              status: "failed",
              file: null,
              provider: shot.clip.audio.provider ?? null,
              error: { code: "VOICE_INTERRUPTED", message: "服务重启导致配音生成中断，可重新生成" }
            };
          }
        }
        // 合成同理：进程内的 running 必然是孤儿
        if (project.compose?.status === "running") {
          project.compose = {
            ...(project.compose || {}),
            status: "failed",
            error: { code: "COMPOSE_INTERRUPTED", message: "服务重启导致合成中断，可重新合成" }
          };
        }
