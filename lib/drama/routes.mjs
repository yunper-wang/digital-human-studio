...
import { createSeriesStore } from "./series.mjs";
...
export async function generateShotFrame(ctx, projectId, shotId, seed) {
  ...
  try {
    ...
    // M8：解析该镜场景/道具参考图 → 上传 ComfyUI → 传 refImage；素材缺失降级纯文本
    let refImage = null;
    ...
    if (cnConfig) {
      ...
      if (refId) {
        const refBytes = ctx.materialStore?.getBytes(refId);
        if (refBytes) {
          try {
            const uploaded = await uploadComfyuiImage({ config: comfyConfig, bytes: refBytes, filename: `${shotId}-ref.png`, fetchImpl });
            refImage = typeof uploaded === "string" ? uploaded : (uploaded?.name || uploaded?.filename || null);
            if (refImage) usedControlnet = true;
          } catch { /* 上传失败降级纯文本 */ }
        }
      }
    }
    // M10：经队列调度 ComfyUI 首帧；无 jobQueue 回退直接执行
    const doFrame = async () => generateFluxFrame({
      config: comfyConfig,
    ...
    const bytes = await (ctx.jobQueue?.enqueue("comfyui", { task: doFrame }) || doFrame());
...
export async function generateShotClip(ctx, projectId, shotId) {
  ...
  try {
    ...
    // M10：视频生成经队列（comfyui kind：剧情镜走 ComfyUI，口播镜走 Seedance 本地适配器，都是本机算力）；无 jobQueue 回退直接执行
    const doClip = async () => {
      if (isDialogue) {
    ...
    await (ctx.jobQueue?.enqueue("comfyui", { task: doClip }) || doClip());
  } catch (error) {
...
export async function generateShotVoice(ctx, projectId, shotId) {
  ...
  const setAudio = (patch) => store.update(projectId, (p) => {
    const shot = p.shots.find((s) => s.id === shotId);
    if (shot) {
      const { voiceRef, ...audioPatch } = patch;
      shot.clip = { ...normalizeClip(shot.clip), audio: { ...normalizeAudio(shot.clip?.audio), ...audioPatch } };
      if (voiceRef) shot.clip.voiceRef = voiceRef; // M8：参考音频注入标记
    }
  });
  const project = store.get(projectId);
  const shot = project?.shots.find((s) => s.id === shotId);
  ...
  try {
    ...
    // M10：口播 TTS 经队列（voice kind）；无 jobQueue 回退直接执行
    const doVoice = async () => synthesizeShotVoice({ voiceTarget: target, text: shot.dialogue, language: "zh", deps: m9AudioDeps, voiceCloneRef });
...
export async function handleDramaApi(request, response, url, ctx) {
  ...
  if (segments[2] === "series" && seriesStore) {
    ...
    if (segments.length === 5 && segments[4] === "projects" && request.method === "POST") {
      ...
      const added = seriesStore.addProject(series.id, String(payload.projectId));
      const joined = store.get(String(payload.projectId));
      if (joined) store.update(joined.id, (p) => { p.seriesId = series.id; });
      return sendJson(response, 200, envelope(true, { series: added }, { requestId }));
    }
    if (segments.length === 6 && segments[4] === "projects" && request.method === "DELETE") {
      const removed = seriesStore.removeProject(series.id, segments[5]);
      const left = store.get(segments[5]);
      if (left && left.seriesId === series.id) store.update(left.id, (p) => { p.seriesId = null; });
      return sendJson(response, 200, envelope(true, { series: removed }, { requestId }));
    }
  ...
  if (segments.length === 4 && segments[2] === "queue" && segments[3] === "status" && request.method === "GET") {
    const empty = { comfyui: { running: 0, queued: 0 }, voice: { running: 0, queued: 0 }, ffmpeg: { running: 0, queued: 0 } };
    return sendJson(response, 200, envelope(true, { queue: ctx.jobQueue?.status() || empty }, { requestId }));
  }
  ...
  if (segments.length >= 4 && segments[2] === "projects") {
    ...
    // M11：成片 ZIP 打包导出（store 模式，mp4+srt+封面+元数据）
    if (segments.length === 6 && segments[4] === "export" && segments[5] === "zip" && request.method === "GET") {
      const exportProject = store.get(projectId);
      if (!exportProject?.compose || exportProject.compose.status !== "succeeded" || !exportProject.compose.file) {
      ...
      const files = [];
      const mp4Path = join(composeDir, exportProject.compose.file);
      if (existsSync(mp4Path)) files.push({ name: `${exportProject.title}.mp4`, bytes: readFileSync(mp4Path) });
      if (exportProject.compose.srtFile) {
        const srtPath = join(composeDir, exportProject.compose.srtFile);
        if (existsSync(srtPath)) files.push({ name: `${exportProject.title}.srt`, bytes: readFileSync(srtPath) });
      }
      if (exportProject.compose.cover) {
        const coverPath = join(composeDir, exportProject.compose.cover);
        if (existsSync(coverPath)) files.push({ name: "cover.png", bytes: readFileSync(coverPath) });
      }
      if (exportProject.compose.meta) {
        const metaPath = join(composeDir, exportProject.compose.meta);
        if (existsSync(metaPath)) files.push({ name: "meta.json", bytes: readFileSync(metaPath) });
      }
      const zip = buildZipBuffer(files);
      response.writeHead(200, {
        "Content-Type": "application/zip",
        "Content-Disposition": `attachment; filename="${encodeURIComponent(exportProject.title)}.zip"`,
    ...
    if (segments.length === 7 && segments[4] === "versions" && segments[6] === "rollback" && request.method === "POST") {
      const ip = request.socket.remoteAddress || "local";
      if (!allowRequest(ip)) return sendJson(response, 429, envelope(false, null, { requestId, errorCode: "RATE_LIMITED", message: "操作太快，请一分钟后再试", retryable: true }));
      if (isPipelineRunning(projectId) || project.compose?.status === "running") {
        return sendJson(response, 409, envelope(false, null, { requestId, errorCode: "ROLLBACK_CONFLICT", message: "流水线或合成运行中，暂不能回滚" }));
      }
